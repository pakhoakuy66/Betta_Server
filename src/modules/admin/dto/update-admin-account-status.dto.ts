...
import {
  ADMIN_ACCOUNT_STATUS_MUTATION_VALUES,
...
export class IssueSuperAdminStatusReauthDto {
  @ApiProperty({
    enum: [AdminAccountStatus.ACTIVE, AdminAccountStatus.LOCKED],
  })
  @IsIn(ADMIN_ACCOUNT_STATUS_MUTATION_VALUES)
...
export class UpdateAdminAccountStatusDto {
  ...
  @ApiProperty({
    minLength: ADMIN_ACCOUNT_STATUS_REASON_NOTE_MIN_LENGTH,
    maxLength: ADMIN_ACCOUNT_STATUS_REASON_NOTE_MAX_LENGTH,
  })
  @Transform(({ value }: { value: unknown }) =>
    typeof value === 'string' ? value.trim() : value,
  )
  ...
  @ApiPropertyOptional({
    description: 'Bắt buộc và chỉ được dùng khi khóa hoặc mở khóa SuperAdmin',
